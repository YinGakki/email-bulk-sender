...
基于 Flask 的 Web 邮件批量发送工具，支持页面配置邮箱、附件上传、分别发送/群发、个性化变量、移动端适配。
...
- **附件上传** - 支持多附件上传发送
- **分别发送 / 群发** - 可选择每位收件人单独收到邮件，或一封邮件群发
- **页面配置邮箱** - 无需修改配置文件，在「设置」页面即可配置 SMTP
...
1. **配置邮箱** - 进入「设置」页面，填写 SMTP 服务器、账号和授权码
2. **导入联系人** - 在「联系人」页面导入 CSV/Excel 文件，或手动添加
3. **创建模板** - 在「模板」页面创建邮件模板（可选）
4. **撰写邮件** - 在「撰写」页面选择收件人、编辑内容、上传附件、选择发送方式
5. **发送邮件** - 点击发送，在「任务」页面查看实时进度
...
在「设置」页面或 `.env` 中设置 `ACCESS_PASSWORD` 即可启用页面密码保护：
...
1. **发送间隔** - 为避免被判定为垃圾邮件，分别发送时每封邮件间隔 1.5 秒，每批次间隔可配置
